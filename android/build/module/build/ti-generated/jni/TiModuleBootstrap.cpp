...
#define TAG "de.marcbender.emitterview"
...
static void TiModule_getBinding(const FunctionCallbackInfo<Value>& args)
{
	...
	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "de.marcbender.emitterview TiModule_getBinding() requires 1 argument: binding");
	...
	if (maybeBinding.IsEmpty()) {
		titanium::JSException::Error(isolate, "de.marcbender.emitterview TiModule_getBinding requires 1 argument: binding. Received argument that could not be converted to a String");
		args.GetReturnValue().Set(scope.Escape(Undefined(isolate)));
		return;
	}
...
Java_de_marcbender_emitterview_TiModuleBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.marcbender.emitterview", &TiModuleBinding);
